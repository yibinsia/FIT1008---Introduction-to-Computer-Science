...
class EffectivenessCalculator:
    ...
    def get_effectiveness(cls, type1: Element, type2: Element) -> float:
        ...
        The best case and worst case is same as O(number of elements), since the number of elements
        has been fixed to 18 in this case, so the best and worst case complexity will be O(1).
        But if the number of elements has changed, then the complexity will be O(n).
        """ 
        #Big-O Notation of this function is O(N), 3 O(1)+O(N)*5 O(1)+ 6 O(1), thus, it is O(N)
        index1 = 0
        index2 = 0
        index = 0

        for name in cls.instance.element_names:
            # print("name = " + name.lower())
            if type1.name.lower() == name.lower():
                index1 = index
            if type2.name.lower() == name.lower():
                index2 = index
            index += 1

        n = len(cls.instance.element_names)
        effectiveness_index = index1 * n + index2
        effectiveness_value = cls.instance.effectiveness_values[effectiveness_index]
